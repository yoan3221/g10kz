...
- 動態視窗歷史：Social 路徑依訊息特性自適應 6–12 條（延續性語境給滿、短獨立訊息縮減）、Reason 路徑固定 12 條，防止 token 無限增長
...
Discord 事件
  ...
  └─ 其他群組訊息              → observe() 寫入 EverOS 長期記憶（≥4字）並靜默返回
```
...
| `POST /api/v1/memory/add` | 每輪結束後（對話）/ 旁觀群組訊息（observe，不 flush） |
| `POST /api/v1/memory/flush` | add 之後（僅對話輪，被動觀察不 flush） |
