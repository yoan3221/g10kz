...
mod tests {
    ...
    fn msgs() -> Vec<Message> {
        vec![
            Message::text(Role::System, "你是小十"),
            Message::text(Role::User, "分析量子纏繞"),
        ]
    }
    ...
    async fn consensus_returns_without_judge() {
        ...
        let (reply, _) =
            fusion_complete(&provider, &msgs(), &CompletionParams::reason("m"), &fusion)
                .await
                .unwrap();
    ...
    async fn exhausted_when_all_fail() {
        ...
        let r = fusion_complete(
            &FailProvider,
            &msgs(),
            &CompletionParams::reason("m"),
            &fusion,
        )
        .await;
        assert!(r.is_err());
    ...
    fn judge_prompt_includes_user_query() {
        ...
        assert!(j
            ...
            .unwrap()
            .text_content()
            .contains("specific question here"));
    }
}
